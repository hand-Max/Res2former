...
class Block(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift_size=0, head_dim_ratio=1., mlp_ratio=4., qkv_bias=False,
                 qk_scale=None,
                 drop=0., attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm,
                 group=8, attn_disabled=False, spatial_conv=False, baseWidth=26, scale=4, attn_mlp=False):
        super().__init__()
...
def Res2former():
    model = res2former(init_channels=24, embed_dim=192, depth=[3, 4, 6, 3], num_heads=[1, 3, 6, 12],
                          attn_stage='0011',
