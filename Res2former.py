...
__all__ = [
    'swin_visformer_small_v2', 'Res2former', 'Block', 'swin_visformer_small_161010', 'swin_visformer_small',
    'swin_visformer_small_16163', 'swin_visformer_small_13183', 'swin_visformer_small_131111',
    'swin_visformer_medium_v2'
]
...
class Bottle2neck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, baseWidth=26, scale=4, stype='normal', group=1, ):
        """ Constructor
        Args:
        ...
        for i in range(self.nums):
            convs.append(nn.Conv2d(width, width, kernel_size=3, stride=stride, padding=1, bias=False, groups=group))
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        self.conv2 = nn.Conv2d(inplanes, inplanes, kernel_size=3,
                               stride=stride, padding=1, groups=inplanes)
        self.bn2 = nn.BatchNorm2d(inplanes)

        self.conv3 = nn.Conv2d(inplanes, planes, kernel_size=1)
        self.bn3 = nn.BatchNorm2d(planes)
        self.gelu = nn.GELU()
...
class Block(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, head_dim_ratio=1., mlp_ratio=4.,
                 qkv_bias=False, qk_scale=None,
                 drop=0., attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm,
                 group=2, attn_disabled=False, spatial_conv=False, baseWidth=26, scale=4, attn_mlp=False):
        super().__init__()
        self.attn_disabled = attn_disabled
        self.attn_mlp = attn_mlp
        self.spatial_conv = spatial_conv
        ...
        if not attn_disabled:
            ...
            # self.bottle2neck = Bottle2neck(int(dim+dim/2), dim, baseWidth=baseWidth, scale=scale, group=group)
            # self.norm2 = norm_layer(int(dim+dim/2))
        self.norm2 = norm_layer(dim)
        # self.bottleneck = nn.Sequential(
        #     Bottleneck(inplanes=dim, planes=int(dim / 2))
        # )
        # self.dwconv = nn.Conv2d(int(dim + dim / 2), dim, kernel_size=1, stride=1, bias=False)
        ...
        if self.attn_mlp:
            ...
            self.bottle2neck = Bottle2neck(dim, dim, baseWidth=baseWidth, scale=scale, group=group)
    ...
    def forward(self, x):
        ...
        if not self.attn_disabled:
            ...
            # x1 = self.bottleneck(shortcut)
            # x = self.dwconv(torch.cat((x, x1), dim=1))
        if self.attn_mlp:
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.bottle2neck(self.norm2(x)))
...
class res2former(nn.Module):
    def __init__(self, img_size=224, patch_size=16, init_channels=32, num_classes=1000, embed_dim=384,
                 depth=[0, 7, 4, 4],
                 ...
                 group=2, pool=True, conv_init=False, embedding_norm=None):
        super().__init__()
        ...
        self.stage0 = nn.ModuleList([
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, group=1,
                attn_disabled=(attn_stage[0] == '0'), spatial_conv=(spatial_conv[0] == '1'), baseWidth=26,
                attn_mlp=False
            )
        ...
        self.stage1 = nn.ModuleList([
            Block(
                ...
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, group=1,
                attn_disabled=(attn_stage[1] == '0'), spatial_conv=(spatial_conv[1] == '1'), baseWidth=52,
                attn_mlp=False
            )
        ...
        self.stage2 = nn.ModuleList([
            Block(
                ...
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, group=4,
                attn_disabled=(attn_stage[2] == '0'), spatial_conv=(spatial_conv[2] == '1'), baseWidth=208,
                attn_mlp=False
            )
        ...
        self.stage3 = nn.ModuleList([
            Block(
                ...
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, group=16,
                attn_disabled=(attn_stage[3] == '0'), spatial_conv=(spatial_conv[3] == '1'), baseWidth=416,
                attn_mlp=False
            )
...
#     model = SwinVisformer(img_size=224, init_channels=16, embed_dim=192, depth=[7, 4, 4], num_heads=3, mlp_ratio=4.,
#                           group=8,
...
#                           mlp_ratio=4., group=2, attn_stage='0011', spatial_conv='1100', norm_layer=BatchNorm,
...
def Res2former(**kwargs):
    model = res2former(img_size=224, init_channels=24, embed_dim=192, depth=[3, 4, 6, 3], num_heads=[1, 3, 6, 12],
                          mlp_ratio=4., group=2, attn_stage='0011', spatial_conv='1100', norm_layer=BatchNorm,
                          conv_init=True, embedding_norm=BatchNorm, **kwargs)
    return model
...
#     model = SwinVisformer(img_size=224, init_channels=32, embed_dim=384, depth=[3, 8, 14, 2], num_heads=[2, 4, 8, 16],
#                           mlp_ratio=4., group=8, attn_stage='1111', spatial_conv='1100', norm_layer=BatchNorm,
#                           conv_init=True, embedding_norm=BatchNorm, **kwargs)
#     return model
